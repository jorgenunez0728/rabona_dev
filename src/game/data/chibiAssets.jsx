...
export const CHIBI = {
  ...
  achievements: {
    ...
    goals500:          { fallback: '⚽' },
    // Gameplay negativo
    racha_negra:       { fallback: '💀' },
    humillacion:       { fallback: '😭' },
    sin_gol_3:         { fallback: '😤' },
    final_perdida:     { fallback: '💔' },
    descenso_epico:    { fallback: '📉' },
    bancarrota:        { fallback: '💸' },
    lesion_masiva:     { fallback: '🏥' },
    abandono:          { fallback: '🚪' },
    // Economía
    tacano:            { fallback: '🏦' },
    comerciante:       { fallback: '🤝' },
    // Secretos
    el_94:             { fallback: '👴' },
    maldito_triple:    { fallback: '☠️' },
    maestro_oscuro:    { fallback: '🌑' },
    no_mercado:        { fallback: '🏘' },
    one_man_army:      { fallback: '🦸' },
    mutador_total:     { fallback: '🔥' },
    speedrun:          { fallback: '⚡' },
    // Arquetipos
    filosofo:          { fallback: '🎭' },
    caudillo_master:   { fallback: '🦅' },
    arquitecto_master: { fallback: '📐' },
    mercenario_master: { fallback: '💰' },
    mistico_master:    { fallback: '🔮' },
    cantera_master:    { fallback: '🌱' },
    apostador_master:  { fallback: '🎲' },
    // Colección
    hall_full:         { fallback: '🏛' },
    arbol_completo:    { fallback: '🌳' },
    todas_cartas:      { fallback: '🎴' },
  },
  legends: {
