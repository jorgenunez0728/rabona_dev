...
export default function TableScreen() {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', background: T.bg }}>
      ...
      <div style={{ flexShrink: 0, padding: '8px 10px 6px', position: 'relative', zIndex: 1 }}>
        <div className="glass" style={{ maxWidth: 420, margin: '0 auto', borderRadius: 10, overflow: 'hidden', border: `1px solid ${T.glassBorder}` }}>
          {/* League header */}
        ...
        {/* Hub action buttons */}
        <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', justifyContent: 'center', maxWidth: 420, margin: '8px auto 0' }}>
          <button className="fw-btn fw-btn-glass" onClick={() => go('roster')} style={{ fontSize: 11, padding: '7px 14px', color: starters.length < 6 ? T.lose : T.tx, borderColor: starters.length < 6 ? T.lose : undefined }}>{starters.length < 6 ? 'Roster' : 'Roster'}</button>
          <button className="fw-btn fw-btn-glass" onClick={() => go('training')} style={{ fontSize: 11, padding: '7px 14px', color: T.win }}>Entrenar</button>
          <button className="fw-btn fw-btn-glass" onClick={openMarket} style={{ fontSize: 11, padding: '7px 14px', color: T.gold }}>Mercado</button>
          <button onClick={() => {
          ...
          }} className={`fw-btn ${(starters.length < 6 && !done) ? '' : done ? (myPos < 2 ? 'fw-btn-primary' : 'fw-btn-danger') : 'fw-btn-green'}`} style={{
            fontSize: 12, padding: '8px 20px',
            opacity: (starters.length < 6 && !done) ? 0.4 : 1,
            cursor: (starters.length < 6 && !done) ? 'not-allowed' : 'pointer',
            ...(starters.length < 6 && !done ? { background: T.bg3, color: T.tx4, border: `1px solid ${T.border}` } : {}),
          }}>
            {starters.length < 6 && !done ? `${starters.length}/6` : done ? (myPos < 2 ? 'Ascender' : 'Resumen') : 'Siguiente'}
          </button>
        </div>
      </div>
      <div style={{ flex: 1, overflow: 'auto', padding: '6px 10px 16px', position: 'relative', zIndex: 1 }}>
        <div style={{ maxWidth: 420, margin: '0 auto' }}>
          {game.coach && (
            <div className="glass" style={{ borderRadius: 10, padding: 12, border: `1px solid ${T.glassBorder}` }}>
              ...
              <div style={{ display: 'flex', gap: 4, marginTop: 8, flexWrap: 'wrap' }}>
                {starters.map(p => (
                  <div key={p.id} onClick={() => setDetailPlayer(p)} style={{ background: `${POS_COLORS[p.pos]}08`, border: `1px solid ${POS_COLORS[p.pos]}20`, borderRadius: 6, padding: '3px 6px', display: 'flex', alignItems: 'center', gap: 3, cursor: 'pointer', transition: 'border-color 0.2s ease' }}>
          ...
          {game.copa?.active && !game.copa?.eliminated && (
            <div className="glass" style={{ borderRadius: 10, padding: 12, border: `1px solid ${T.gold}20`, marginTop: 8 }}>
