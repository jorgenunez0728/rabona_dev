...
import { SFX, Crowd } from "../src/game/audio";
import {
      ...
      <div style={{ display: 'flex', flexDirection: 'column', height: '100%', background: T.bg }}>
        <div style={{ flexShrink: 0, padding: '8px 10px', background: T.bg, borderBottom: `1px solid ${T.border}` }}>
          <div style={{ display: 'flex', gap: 4, maxWidth: 420, margin: '4px auto 0', flexWrap: 'wrap' }}>
            ...
            <button className={`fw-btn ${starters.length === 5 ? 'fw-btn-green' : 'fw-btn-outline'}`} onClick={() => starters.length === 5 && go('prematch')} disabled={starters.length < 5} style={{ fontSize: 10, padding: '5px 12px' }}>Previa</button>
          </div>
          {starters.length < 5 && <div style={{ fontSize: 10, color: T.lose, textAlign: 'center', marginTop: 3 }}>⚠ Necesitas 5 titulares ({starters.length}/5)</div>}
        </div>
        <div style={{ flex: 1, overflow: 'auto', padding: '6px 10px 16px' }}>
          <div style={{ maxWidth: 420, margin: '0 auto' }}>
    ...
    async function runSim() {
      ...
      while (S.minute < 90) {
        ...
        if (S.possession) {
          ...
          const tD = avgStat(starters, 'def') + (noFouls ? 2 : 0);
      ...
      setRewards({ options: rwOptions, selected: null, stolen, result: { ps, rs, won, drew, lost, xpGain, coinGain: coinGain + objCoins, rivalName: S.rivalName, rosterSnapshot: snapRoster, rivalPlayers: S.rivalPlayers, starters: allRoster.filter(p => p.role === 'st'), goals, cards, possPct, shots: S.shots, morale: S.morale, objResults, personalityEvents, injuryList, socialPosts, matchType } });
